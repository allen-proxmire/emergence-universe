#!/usr/bin/env python
"""
generate_law_x_sweeps.py
==========================
Law X Consolidation Sweep: generate sweep + atlas files to test the angle
partition, true IC boundary, boundary-layer phenomenon, and taxonomy freeze.

Extensions beyond prior coverage:
    A. Non-canonical angles near the boundary layer:
       -0.75, -0.5, -0.25, +0.25, +0.75  (many already exist from prior sweeps)
       PLUS angles beyond the +10 grid saturation limit: +12, +15
    B. Non-canonical radii: 0.025, 0.125, 0.275, 0.525, 0.675, 0.825
...
# Law X extensions
ANGLES_BOUNDARY = [-0.75, -0.5, -0.25, 0.25, 0.75]   # boundary layer angles
ANGLES_BEYOND   = [12, 15]                              # beyond +10 grid saturation
ANGLES_NEW      = ANGLES_BOUNDARY + ANGLES_BEYOND

RADII_NEW   = [0.025, 0.125, 0.275, 0.525, 0.675, 0.825]
DRIFTS_NEW  = [0.003, 0.007, 0.013, 0.017, 0.030]

ALL_ANGLES = sorted(set(ANGLES_CANONICAL + ANGLES_NEW))
ALL_RADII  = sorted(set(RADII_CANONICAL + RADII_NEW))
ALL_DRIFTS = sorted(set(DRIFTS_CANONICAL + DRIFTS_NEW))
...
def build_extended_grid():
    ...
    Includes all cross-combinations involving at least one new parameter,
    but we rely on the skip-if-exists logic to avoid regeneration.
    """
    cells = set()

    # A: New angles x all N x canonical radii x canonical drifts
    for n in NS_ALL:
        for a in ANGLES_NEW:
    ...
    # B: New radii x all N x ALL angles (including boundary) x canonical drifts
    for n in NS_ALL:
        for a in ALL_ANGLES:
            for r in RADII_NEW:
                for d in DRIFTS_CANONICAL:
    ...
    # C: New drifts x all N x ALL angles x canonical radii
    for n in NS_ALL:
        for a in ALL_ANGLES:
            for r in RADII_CANONICAL:
                for d in DRIFTS_NEW:
    ...
    # D: Boundary-layer cross-products: boundary angles x new radii x new drifts
    #    (ensures dense coverage of the boundary layer at non-canonical params)
    for n in NS_ALL:
        for a in ANGLES_BOUNDARY:
            for r in RADII_NEW:
                for d in DRIFTS_NEW:
                    cells.add((n, a, r, d))
...
def main():
    ...
    print("Law X Consolidation Sweep Generator")
    print("=" * 60)
    print(f"  N values:         {NS_ALL}")
    print(f"  Boundary angles:  {ANGLES_BOUNDARY}")
    print(f"  Beyond-grid:      {ANGLES_BEYOND}")
    print(f"  New radii:        {RADII_NEW}")
    print(f"  New drifts:       {DRIFTS_NEW}")
    ...
    print()
    print("=" * 60)
    print("  LAW X CONSOLIDATION SWEEP SUMMARY")
    print("=" * 60)
    print(f"  Total grid cells:      {total}")
    ...
    print(f"  Run check_law_x_consolidation.py to analyze results.")
