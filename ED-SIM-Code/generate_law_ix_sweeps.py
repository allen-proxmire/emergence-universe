#!/usr/bin/env python
"""
generate_law_ix_sweeps.py
==========================
Law IX Consolidation Sweep: generate sweep + atlas files at non-canonical
parameter values to test whether lambda(N) = spacing / merge_thr predicts
the unified manifold at previously unseen points in the parameter space.

Three extensions:
    A. Non-canonical radii:   0.025, 0.125, 0.275, 0.525, 0.675, 0.825
    B. Non-canonical angles:  -7, -3, -0.5, +0.5, +3, +7
    C. Non-canonical drifts:  0.003, 0.007, 0.013, 0.017, 0.030

All tested at all canonical + extended N values (4,8,12,20,24,28,32,40,48,
56,64,80,96,128), plus the canonical angles/radii/drifts as needed for
cross-combination coverage.

Generates BOTH sweep.json and atlas.json for each new combination.
Reuses the exact simulation engine from all prior generators.
"""
...
# Canonical values (already exist in prior sweeps)
...
def build_extended_grid():
    """
    Return list of (N, angle, radius, drift) tuples that are NEW --
    i.e., involve at least one non-canonical parameter value.
    """
    # We want cross-combinations so each extension is tested at
    # canonical values of the other dimensions.

    extended = set()

    # A: Non-canonical radii x all N x canonical angles x canonical drifts
    for n in NS_ALL:
    ...
    # B: Non-canonical angles x all N x canonical radii x canonical drifts
    for n in NS_ALL:
        for a in ANGLES_NEW:
    ...
    # C: Non-canonical drifts x all N x canonical angles x canonical radii
    for n in NS_ALL:
        for a in ANGLES_CANONICAL:
            for r in RADII_CANONICAL:
                for d in DRIFTS_NEW:
...
def angle_label(ang):
    ...
    Fractional angles: p0p5deg, m0p5deg
    """
    if ang == 0:
        return "0deg"
...
def main():
    ...
    print("Law IX Consolidation Sweep Generator")
    print("=" * 60)
    print(f"  N values:        {NS_ALL}")
    print(f"  New radii:       {RADII_NEW}")
    print(f"  New angles:      {ANGLES_NEW}")
    print(f"  New drifts:      {DRIFTS_NEW}")
    print(f"  Total new cells: {total}")
    print()
    ...
    print()
    print("=" * 60)
    print("  LAW IX CONSOLIDATION SWEEP SUMMARY")
    print("=" * 60)
    ...
    print(f"  Run check_law_ix_consolidation.py to analyze results.")
