#!/usr/bin/env python
"""
generate_high_n_sweeps.py
==========================
High-N Asymptotic Probe: generate sweep + atlas files for N >> 32
to test whether ED-Arch exhibits a true asymptotic law as N -> infinity.

Parameter space:
    N:      40, 48, 56, 64, 80, 96, 128
    Angles: -10, -5, -2, -1, 0, +1, +2, +5, +10
    Radii:  0.05 ... 0.95  (11 values)
    Drifts: 0.00, 0.01, 0.02

Generates BOTH sweep.json and atlas.json for each combination.
Reuses the exact simulation engine and conventions from the canonical generators.
"""
...
def main():
    total = len(NS_HIGH) * len(ANGLES) * len(RADII) * len(DRIFTS)

    print("High-N Asymptotic Probe Generator")
    print("=" * 60)
    print(f"  N values:  {NS_HIGH}")
    ...
    for ni, n in enumerate(NS_HIGH):
        ...
        rate = done / total_elapsed if total_elapsed > 0 else 1
        eta = remaining / rate if rate > 0 else 0
        print(f"  N={n:3d} complete  ({done}/{total}, "
              f"{100*done/total:.0f}%, {n_elapsed:.0f}s this N, "
    ...
    print()
    print("=" * 60)
    print("  HIGH-N GENERATION SUMMARY")
    print("=" * 60)
    print(f"  Total combinations:    {total}")
    ...
    print(f"  Run check_high_n_asymptotic.py to analyze results.")
